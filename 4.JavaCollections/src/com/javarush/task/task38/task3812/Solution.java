...
if (c.isAnnotationPresent(PrepareMyTest.class)){
    PrepareMyTest prep= (PrepareMyTest)c.getAnnotation(PrepareMyTest.class);
    String[]q =  prep.fullyQualifiedNames();
    for(String g:q){
    ...
    public static boolean printValues(Class c){
        if (c.isAnnotationPresent(PrepareMyTest.class)){
            ...
            Class[] q =  prep.value();
            for(Class g:q){
                System.out.println(g.getSimpleName());
            }
            return true;
        } else
            return  false;
...
В методы printFullyQualifiedNames и printValues приходит класс. Если этот класс отмечен аннотацией PrepareMyTest, необходимо вывести на экран fullyQualifiedNames и values в соответствующих методах и вернуть true.
