...
public class Solution {
    public static void main(String[] args) throws IllegalAccessException, InstantiationException, ClassNotFoundException {
        Set<? extends Animal> allAnimals =
                getAllAnimals(Solution.class.getProtectionDomain().getCodeSource().getLocation().getPath() + Solution.class.getPackage().getName().replaceAll("[.]", "/") + "/data");
    ...
    public static Set<? extends Animal> getAllAnimals(String pathToAnimals) throws ClassNotFoundException, IllegalAccessException, InstantiationException {
        Set<Animal> h = new HashSet<>();
        //   System.out.println(pathToAnimals);
       //  String h = pathToAnimals.replaceAll("/","\\\\").substring(1);
      //  String g="c:\\Users\\mr_ma\\Documents\\Dasha\\jv\\JavaRushTasks\\out\\production\\4.JavaCollections\\com\\javarush\\task\\task35\\task3507\\data\\";
       // System.out.println(h);
        //System.out.println(g);
      //  File f = new File(h);
        //File f = new File(pathToAnimals);
        // String[] allClasses = f.list();
        File[] list = new File(pathToAnimals).listFiles();
        //System.out.println(allClasses);
       // String[] q=new String[1];
        //q[0]=pathToAnimals.substring(1)+"/";
        for(File sss:list){
           //  System.out.println(pathToAnimals+"/"+sss);
          // System.out.println(  ClassLoader.getSystemClassLoader().loadClass(pathToAnimals.substring(1)+"/"+sss).newInstance() );
         // System.out.println(  Class.forName(pathToAnimals.substring(1)+"/"+sss ));
           // DynamicClassOverloader loader= new DynamicClassOverloader();
            // текущий каталог "." будет единственным каталогом поиска
          //  Class clazz= Class.forName(sss ,true,loader);
            String packageName = Solution.class.getPackage().getName() + ".data";
            Class clazz = new DynamicClassOverloader().load(sss.toPath(), packageName);
            //Class clazz = loader.load(Paths.get(pathToAnimals+"/"+sss), sss);
