...
public class Solution {
    public static void main(String[] args) throws IOException {
      //  String filename = "c:\\Users\\mr_ma\\Documents\\Даша\\java\\1\\1.txt";//args[0];
//        String pathZip = "c:\\Users\\mr_ma\\Documents\\Даша\\java\\1\\2.zip";//args[1];//
        String filename = args[0];
        String pathZip = args[1];
        // Path tmpArch=Files.createTempDirectory("tmp_");
        Path pathZipPath = Paths.get(pathZip);
        ...
        try (ZipOutputStream zipOutputStream = new ZipOutputStream(Files.newOutputStream(tempZipFile))) {
            try (ZipInputStream zipInputStream = new ZipInputStream(Files.newInputStream(Paths.get(pathZip)))) {
            ...
            Path fullPath = tempZipFile.resolve(Paths.get("new\\" + (Paths.get(filename).getFileName()).toString()));


                ZipEntry entry = new ZipEntry("new\\" + (Paths.get(filename).getFileName()).toString());
         ...
         //   }
            ...
            Files.move(tempZipFile, pathZipPath, StandardCopyOption.REPLACE_EXISTING);
        }
    }
